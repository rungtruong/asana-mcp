...
server.tool(
  ...
  async ({ parentTaskId, name, description, dueDate, assignee }) => {
    ...
    try {
      if (asanaApi) {
        // Call the actual Asana API
        const result = await asanaApi.createSubtask(parentTaskId, name, description, dueDate, assignee);
        console.debug("Subtask created with ID:", result.data.gid);
        
        return {
          content: [{ 
            type: "text", 
            text: `Subtask "${name}" created successfully with ID: ${result.data.gid}` 
          }]
        };
      } else {
        // Stub implementation
        return {
          content: [{ 
            type: "text", 
            text: `Subtask "${name}" created successfully for parent task ${parentTaskId} (stub)` 
          }]
        };
      }
    } catch (error: any) {
      ...
      return {
        content: [{ 
          type: "text", 
          text: `Error creating subtask: ${error.message || 'Unknown error'}` 
        }]
      };
    }
  }
  ...
  async ({ parentTaskId }) => {
    ...
    try {
      ...
      return {
        content: [{ 
          type: "text", 
          text: `Error listing subtasks: ${error.message || 'Unknown error'}` 
        }]
      };
    }
  }
...
server.tool(
  ...
  async ({ subtaskId, name, description, dueDate, assignee, completed }) => {
    ...
    try {
      if (asanaApi) {
        ...
        return {
          content: [{ 
            type: "text", 
            text: `Subtask ${subtaskId} updated successfully` 
          }]
        };
      } else {
        // Stub implementation
        return {
          content: [{ 
            type: "text", 
            text: `Subtask ${subtaskId} updated successfully (stub)` 
          }]
        };
      }
    } catch (error: any) {
      ...
      return {
        content: [{ 
          type: "text", 
          text: `Error updating subtask: ${error.message || 'Unknown error'}` 
        }]
      };
    }
  }
      ...
      if (asanaApi) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Subtask ${subtaskId} deleted successfully`
            }
          ]
        };
      } else {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Subtask ${subtaskId} deleted successfully (stub implementation)`
            }
          ]
        };
      }
