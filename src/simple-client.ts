...
async function main() {
  ...
  if (args.length === 0) {
    console.log("Usage:");
    console.log(
      '  node dist/simple-client.js create-task "Task Name" "Description" "Due Date" "Assignee" "Project"'
    );
    console.log('  node dist/simple-client.js list-tasks ["ProjectId"]');
    console.log(
      '  node dist/simple-client.js update-task "TaskId" "New Name" "New Description" "New Due Date" "New Assignee" "true|false"'
    );
    console.log('  node dist/simple-client.js delete-task "TaskId"');
    console.log(
      '  node dist/simple-client.js create-project "Project Name" ["Notes"] ["Color"] ["Is Public"]'
    );
...
async function createProject(client: Client, args: string[]) {
  ...
  const result = await client.callTool({
    ...
    arguments: {
      ...
      color: args[3] || null,
      isPublic:
        args[4] === "true" ? true : args[4] === "false" ? false : undefined,
    },
  });
